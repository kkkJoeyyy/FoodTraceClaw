# Food Chat Capability

## ADDED Requirements

### Requirement: Chat Intent Routing

The system SHALL classify incoming chat messages into one of four intents: extract, query, more, or other.
...
And the system SHALL return top 5 stores near the specified location
...
The system SHALL expose a POST /api/chat endpoint.

#### Scenario: Chat with text

Given POST /api/chat with `{"message": "成都美食？", "session_id": "abc"}`
Then the response SHALL include intent, reply text, and any recommended stores

#### Scenario: Chat with image

Given POST /api/chat with an image
Then the system SHALL perform VL extraction and return results
